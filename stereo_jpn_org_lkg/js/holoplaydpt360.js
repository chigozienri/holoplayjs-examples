...
function HoloPlay(scene, camera, renderer, focalPointVector, constantCenter, hiResRender){
    ...
    var startTime;
    var HSMes=0;
    var nRev=0; 	//1 -> Black:front White;back
...
//        var newTilt = screenH / (screenW * slope);
    ...
    HoloPlay.prototype.setSize = function(width, height) {
        renderer.setSize(width, height);
        var pixelRatio = renderer.getPixelRatio();
        bufferSceneRender.setSize(width * pixelRatio, height * pixelRatio);
        bufferSceneRenderd.setSize(width * pixelRatio, height * pixelRatio);
    };
...
//	fromjsfnc(e.keyCode);
    ...
    HoloPlay.prototype.sendkeycode = function (Keynum){
        if(Keynum === 220){
            threeD = !threeD;
        }
        if(Keynum === 13){
